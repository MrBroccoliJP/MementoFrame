...
<section class="card"><h2>Appearance</h2><form method="post" action="/save_appearance_settings"><label>Theme</label><select name="display_theme"><option value="classic" {'selected' if config.get('display_theme') == 'classic' else ''}>Classic</option><option value="minimal" {'selected' if config.get('display_theme') == 'minimal' else ''}>Minimal sidebar</option></select><label>Forecast icon style</label><select name="weather_icon_pack">{''.join(f'<option value="{pack}" {"selected" if config.get("weather_icon_pack") == pack else ""}>{pack.title()}</option>' for pack in ('fill','flat','line','monochrome'))}</select><button>Save appearance</button></form></section>
...
def mock_weather():
    ...
    state["weather"].update({
        ...
        "precipitationProbability": int(float(request.form.get("precipitationProbability", 0) or 0)),
