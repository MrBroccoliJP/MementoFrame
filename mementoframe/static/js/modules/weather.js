...
function getActiveWeatherAlert() {
  ...
  const cyclePosition = Date.now() % WEATHER_ALERT_CYCLE_MS;
  if (cyclePosition >= WEATHER_ALERT_VISIBLE_MS) return null;

  // During the 1-minute alert window, rotate through every active alert
  // instead of pinning the display to the first one forever.
  const alertSlotMs = Math.max(10000, WEATHER_ALERT_VISIBLE_MS / currentWeatherAlerts.length);
  const index = Math.floor(cyclePosition / alertSlotMs) % currentWeatherAlerts.length;
  return currentWeatherAlerts[index] || currentWeatherAlerts[0];
}
