...
REPAIR_HELPER="${APP_DIR}/repair_services.sh"
...
# Enable I2C and select the DS3231 RTC without replacing unrelated dtparam or
# dtoverlay directives that may already exist in the [all] section.
...
log "Writing service repair helper"
cat > "${REPAIR_HELPER}" <<'EOF_REPAIR_HELPER'
#!/usr/bin/env bash
set -euo pipefail
...
EOF_REPAIR_HELPER
chown root:root "${REPAIR_HELPER}"
chmod 0755 "${REPAIR_HELPER}"
...
log "Writing systemd service: mementoframe-updater.service"
cat > "${UPDATER_SERVICE}" <<EOF_UPDATER_SERVICE
[Unit]
Description=MementoFrame automatic update check/install
After=network-online.target NetworkManager.service
...
log "Writing systemd timer: mementoframe-updater.timer"
cat > "${UPDATER_TIMER}" <<EOF_UPDATER_TIMER
[Unit]
Description=Run MementoFrame updater hourly
...
systemctl daemon-reload
systemctl disable --now mementoframe-post-reboot.service 2>/dev/null || true
...
systemctl restart mementoframe-updater.timer
...
echo "Updater logs: journalctl -u mementoframe-updater.service -n 80 --no-pager"
echo "Updater timer: systemctl list-timers mementoframe-updater.timer"
